...
namespace TrafficDemo.Classes
{
    ...
    class Road
    {
        ...
        public void AddPedestrians(Lane lane, int amount)
        {
            foreach (string group in this.lanesGroups.Keys)
            {
                if (this.lanesGroups[group].Contains(lane) && lane.IsSidewalkLane)
                {
                    float distanceBetweenPedestrian = 0;
                    for (int i = 0; i < amount; i++)
                    {

                        var sidewalkLane = this.lanesGroups[group].Where(x => x.IsSidewalkLane).ToArray();
                        int dice = new Random().Next(0, sidewalkLane.Count());
                        Console.WriteLine(group + " - " + dice);
                        Lane l = sidewalkLane[dice];
                        Pedestrian p = new Pedestrian(Color.Yellow, 0, 0, 0, 0, l);

                        if (amount == 1)
                        {
                            foreach (Lane laneInGroup in this.lanesGroups[group])
                            {
                                if (laneInGroup.Pedestrians.Count > 0)
                                {
                                    if (laneInGroup.LaneDirectionFrom == Direction.North)
                                    {
                                        float distance = laneInGroup.Pedestrians.Last().Location.Y - laneInGroup.StartPoint.Y;
                                        if (distance < 10) distanceBetweenPedestrian += Math.Abs(15 - distance);
                                    }
                                    else if (laneInGroup.LaneDirectionFrom == Direction.South)
                                    {
                                        float distance = laneInGroup.StartPoint.Y - laneInGroup.Pedestrians.Last().Location.Y;
                                        if (distance < 10) distanceBetweenPedestrian += Math.Abs(15 - distance);
                                    }
                                    else if (laneInGroup.LaneDirectionFrom == Direction.West)
                                    {
                                        float distance = laneInGroup.Pedestrians.Last().Location.X - laneInGroup.StartPoint.X;
                                        if (distance < 10) distanceBetweenPedestrian += Math.Abs(15 - distance);
                                    }
                                    else if (laneInGroup.LaneDirectionFrom == Direction.East)
                                    {
                                        float distance = laneInGroup.StartPoint.X - laneInGroup.Pedestrians.Last().Location.X;
                                        if (distance < 10) distanceBetweenPedestrian += Math.Abs(15 - distance);
                                    }
                                }
                            }
                        ...
                        if (l.LaneDirectionFrom == Direction.North)
                        {
                            p = new Pedestrian(Color.Black, l.StartPoint.X, l.StartPoint.Y - distanceBetweenPedestrian, 6, 6, l);
                            l.Pedestrians.Add(p); // Create the next car 10 pixels behind 
                        }
                        else if (l.LaneDirectionFrom == Direction.South)
                        {
                            p = new Pedestrian(Color.Black, l.StartPoint.X, l.StartPoint.Y + distanceBetweenPedestrian, 6, 6, l);
                        ...
                        {
                            p = new Pedestrian(Color.Black, l.StartPoint.X - distanceBetweenPedestrian, l.StartPoint.Y, 6, 6, l);
                            l.Pedestrians.Add(p); // Create the next car 10 pixels behind 
                        }
                        else if (l.LaneDirectionFrom == Direction.East)
                        {
