...
namespace TrafficDemo.Classes
{
    ...
    class Mediator
    {
        ...
        public void SpawnEmergencyVehicle(Point start,  Point target ,List<Node> path)
        {
            Console.WriteLine("We got the dict " + path.Count);

            this.EV = new EmergencyVehicle(Color.Blue, start.X, start.Y, 15, 15, this, path, target);

        }

       /* public string EVMessage(EmergencyVehicle sender, string message)
        {
            string response = "";
            foreach(Road R in this.controller)
            return response;
        }*/
        ...
        public string SendMessage(string Message, Car Sender, PointF target)
        {
            ...
            if (Sender.Location == Sender.CurrentLane.EndPoint)
            {
                ...
                foreach (Road R in controlledRoads)
                {
